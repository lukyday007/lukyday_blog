---
title: 네트워크 4편 - 로드밸런서, 정보로 나누는 트래픽
...
경제학자 조지 애컬로프는 1970년 논문 "레몬 시장"에서 이 구조를 <strong>정보 비대칭(Information Asymmetry)</strong>이라 언급했다. 중고차 시장에서 판매자는 차의 결함을 알지만 구매자는 모를 때, 이 정보의 격차 하나가 시장 전체를 왜곡한다. 

<div style="text-align: right; margin-top: -0.5rem;">
  <a href="https://en.wikipedia.org/wiki/The_Market_for_Lemons">Information Asymmetry</a><br>
</div>

DNS 라운드로빈도 같다. DNS 서버는 서버 A가 과부하 상태임을 알고 있지만, 클라이언트는 TTL이 만료되기 전까지 그 정보를 얻을 수 없다. 분산이 왜곡되는 것은 캐싱의 문제가 아니라, 정보를 가진 자와 가지지 못한 자 사이의 구조적인 '단절' 때문이다..
...
L4는 정보 격차를 받아들이는 전략이다. 패킷 안에 무엇이 담겼는지 모른 채로 결정을 내린다.
DNS 라운드로빈이 정보 부재 때문에 실패했다면, L4는 정보 부재를 의도적 선택으로 바꾼다.
차이는 명확하다. DNS는 정보가 없어서 잘못 배분하고, L4는 정보를 포기한 대가로 속도를 얻는다.
...
URL을 읽을 수 있으니 `/api/payments`는 결제 서버로, `/api/products`는 상품 서버로 보낼 수 있다. 쿠키 또한 확인이 가능해 <strong>세션 고정(Session Persistence)</strong>도 가능하다. 
쇼핑몰에서 장바구니에 담은 상품 정보가 서버 A에만 저장되어 있다면, 그 사용자는 계속 서버 A로 가야 장바구니가 유지된다. L7은 쿠키 안의 사용자 ID를 읽고 매번 같은 서버로 보낸다. 

L7은 정보 격차를 비용을 치르고 메우는 전략이다. 2편에서 본 거래비용이 여기서도 적용된다. 정보를 얻는 행위 자체에 비용이 따른다. 패킷을 열고, 헤더를 파싱하고, URL을 해석하는 과정은 모두 정보를 얻기 위한 거래 비용이다. 그 비용을 치르는 대신, L4가 할 수 없는 판단을 한다. 
...
세 가지 시스템이 같은 문제 앞에서 서로 다른 답을 했다.

DNS 라운드로빈은 정보를 가지지 못해서 실패했다. 서버의 상태를 모른 채 순번만 돌렸기 때문이다. 이러한 정보 격차가 분산을 왜곡시켰다.

L4는 정보를 포기하기로 선택했다. 모르는 채로 결정하는 대신, 그 무지를 속도로 환산했다. 정보 격차를 자산으로 만든 것이다.

L7은 정보를 사들이기로 선택했다. 파싱과 해석에 시간을 지불하고 정확도를 얻었다. 정보 격차를 비용으로 메운 것이다.

애컬로프가 보여준 것은 정보 격차 그 자체가 아니라, **격차를 다루는 방식이 결과를 가른다는 사실**이다. 
중고차 시장에서 격차를 무시한 시장은 무너졌고, 보증서로 격차를 메운 시장은 살아남았다.

로드밸런서도 같다. 격차를 무시하면 DNS가 되고, 받아들이면 L4가 되고, 메우면 L7이 된다.

**문제는 정보가 있냐 없냐가 아니다. 그 격차를 어떻게 처리할 것이냐다.**

네트워크 1편부터 반복해온 구조다. 골드랫은 제약이 어디 있는지를 봤고, 코스와 윌리엄슨은 거래 비용을 감수하는 조건을 설명했고, 애컬로프는 정보의 격차가 행동을 어떻게 갈라놓는지를 보였다. 네 편에 걸쳐 다른 이름으로 반복된 질문은 결국 하나다. 
...
**병목이 어디인지 알면, 어디서 나눠야 하는지 알 수 있다.** 
**정보를 어떻게 다룰지 알면, 어떻게 나눠야 하는지도 알 수 있다.**
