...
description: "parallelStream으로 메시지 전달을 병렬화했지만 병목은 해소되지 않았다. 호출 대상을 유저에서 서버로 바꾸고, 배치 처리 후 병렬화를 적용한 과정과 벤치마크 결과를 정리한다."
sidebar:
  order: 7
date: 2026-07-06 
...
병렬 처리는 실행을 최적화한다. 배치 처리는 작업량 자체를 최적화한다.


## 선택 — 먼저 배치, 그 다음 병렬

배치와 병렬은 양자택일이 아니다. 순서의 문제다.
...
### sendMessageToChatRoom 변경 — 서버 단위 배치 릴레이

유저별 루프를 분류 → 배치 → 릴레이 구조로 변경했다.
...
### 시나리오 1 — 개별 처리 vs 배치 처리

원격 유저 200명이 모두 같은 서버에 연결된 상황. 개별 호출과 배치 호출의 차이를 측정했다.
...
같은 200명에게 메시지를 전달하는데, 실행 방식만 바꿔도(병렬화) 36ms, 작업량을 줄이면(배치) 1.2ms.

### 시나리오 2 — 배치 이후 병렬화는 언제 의미를 갖는가

시나리오 1에서 원격 서버가 1대일 때, 배치 처리에 스레드 풀을 추가해도 효과가 없었다. gRPC 호출이 1번뿐이니 병렬화할 대상이 없기 때문이다.
...
| 7편 | 메시지 릴레이 최적화 | 호출 대상이 유저가 아니라 서버 | 개별 처리 대신 서버별 배치 |
